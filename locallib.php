...
class assign_submission_youtube extends assign_submission_plugin {
    ...
    */
    ...
    public function get_form_elements($submission, MoodleQuickForm $mform, stdClass $data) {
		...
		$manual .= "<input type=\"text\" size=\"75\" class='youtube_url' onchange=\"document.getElementById('id_manualurl').value=this.value;\"/>";
		$automatic_url .= "
			<script language=\"javascript\" type=\"text/javascript\">
			<!--
			function popitup(url) {
				newwindow=window.open(url,'name','height=500,width=500,top=30,left=400');
			...
			<input type=\"button\" value=\"Add Submission\" onclick=\"popitup('submission/youtube/my_uploads.html')\"/>";
		...
		if(get_config('assignsubmission_youtube', 'allow_manual')){ 
				...
				$mediadivs .= '<div id="tabmanual">' . $manual . $automatic_url . '</div>';
		}
